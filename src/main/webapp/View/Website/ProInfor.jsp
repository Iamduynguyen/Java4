...
<div id="page" class="site boxed-layout">
  ...
  <div class="container">
    <div class="row mx-auto my-4 p-left-10 rounded" style="background-color: lavender">
      <div class="col">
        <div class="rounded mr-3"  style="text-align: center">
          <img class="mt-2" width="380px" src="<c:url value="/image/${product.img1}"/>">
          <div class="row px-xl-5 mt-4 mb-2">
            <div class="col"></div>
            <div class="col">
              <img width="80px" src="<c:url value="/image/${product.img2}"/>">
            </div>
            <div class="col">
              <img width="80px"  src="<c:url value="/image/${product.img3}"/>">
            </div>
            <div class="col"></div>
          </div>
        </div>
      ...
      <div class="col">
        <div class="ml-2 pl-5 mr-3 rounded h-100">
          <div class="ml-2 my-2">
            <br>
            <img width="80px" src="https://i1.wp.com/benhvienungbuoudanang.com.vn/wp-content/uploads/2020/01/new-icon.gif?fit=368%2C333&ssl=1">
            <br>
            <br>
            <h2>${product.name}</h2>
            <br>
            <h4>${product.catalog.name}</h4>
            <br>
            <p>
              As noted above, each of these breakpoints have their own container, unique class prefix, and modifiers. Here’s how the grid changes across these breakpoints:
            </p>
            <br>
            <h2 style="color: red">${product.price} vnd</h2>
            <br>
            <div class="row mb-2">
              <form method="post" class="col row">
                <input type="number" name="number" min="1" max="${product.quanlity}" class="ml-2 col form-control" required>
                <button type="submit"  class="ml-2 col" style="background-color: blue">Mua hàng</button>
              </form>
              <div class="col row mx-2">
                <c:if test="${like}">
                  <a href="<c:url value="/infor/?id=${product.id}&like=false"/>">
                    <img width="40px" src="https://img2.thuthuatphanmem.vn/uploads/2019/02/22/trai-tim-lap-lanh_121246613.gif"> Đã thích
                  </a>
                </c:if>
                <c:if test="${like==false}">
                  <a href="<c:url value="/infor/?id=${product.id}&like=true"/>">
                    <img width="40px" src="https://img2.thuthuatphanmem.vn/uploads/2019/02/22/trai-tim-lap-lanh_121246613.gif"> Yêu thích
                  </a>
                </c:if>
                <div class="col"></div>
              </div>
    ...
    <div class="col-md-12" style="text-align: center">
      <h2 class="mt-5 mx-auto mb-3" style="color: blue">Bạn có thể lựa chọn thêm</h2>
    </div>
    <div class="mx-auto row m-bottom-10" style="height: 150px; background-color: whitesmoke">
        <c:forEach var="pro" items="${lstfilter}">
          <div class="col mx-2 rounded px-1 py-1" style="background-color: lavender;width: 120px;height: 150px;text-align: center">
              <a href="<c:url value="/infor/?id=${pro.id}"/>">
                <div class="row">
                  <img width="100" src="<c:url value="/image/${pro.img1}"/>"/>
                </div>
                <div class="row">
                  <p  style="color: #ff0080">${pro.name}</p>
                </div>
                <div  class="row">
                  <p style="color: blue">${pro.price-pro.discount} vnd</p>
                </div>
              </a>
          </div>
        </c:forEach>
